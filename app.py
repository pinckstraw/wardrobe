...
def get_gemini_recommendation(api_key, selected_item, wardrobe_items, season, occasions):
    try:
        ...
        prompt = (
            ...
            f"3. 顏色混搭規定：使用者提供的清單中包含了他們指定的特定顏色。請『務必』將清單中出現的不同顏色混搭在一起（例如清單中若同時有藍跟白，就必須搭出藍白配色），『絕對不要』只挑選單一顏色完成全套！\n\n"
            ...
            f'  "分類名稱B": {{"fname": "正確的檔名.png", "reason": "妳的韓系配色或風格巧思"}}\n'
            f"}}"
        )
    ...
    if not cat_names:
        ...
        with st.container():
            ...
            if sel is None and not st.session_state.recs:
                ...
                # 重新推薦按鈕
                if st.session_state.recs and "_error" not in st.session_state.recs:
                    if st.button("🔄 重新推薦", key="redo_rec", width="stretch"):
        ...
        if sel is not None and not st.session_state.browse_open:
            ...
            with cb:
                if st.button("🔄\n更換", key="change_item", width="stretch"):
    ...
    if not cat_names:
        ...
        if step == "upload":
            ...
            if "saved_img_id" in st.session_state:
                ...
                # 重新上傳按鈕
                if st.button("🔄 重新上傳照片", key="reupload", width="stretch"):
            ...
            if not sticker_bytes:
                ...
                with col_back:
                    if st.button("← 重新上傳", key="back_to_upload", width="stretch"):
                        ...
                        st.rerun()
                with col_save:
                    if st.button("🎀 儲存到衣櫥！", key="save_sticker", width="stretch"):
                        if not item_name:
                            st.error("請填寫衣物名稱！")
        ...
        if not files_to_show:
            ...
            for f_cat, fname in files_to_show:
                ...
                if is_editing:
                    ...
                    with sv_col:
                        if st.button("💾 儲存修改", key=f"save_edit_{fname}", width="stretch"):
                            # 更新 metadata
                ...
                # ── 一般顯示模式 ──
                    ...
                    with c3:
                        if st.button("✏️", key=f"edit_{fname}", width="stretch"):
                            ...
                            st.rerun()
                    with c4:
                        if st.button("🗑️", key=f"del_{fname}", width="stretch"):
...
# ═══════════════════════════════════════════════════════
    ...
    if st.button("💾 儲存 API Key", width="stretch"):
    ...
    if cat_names:
        ...
        for cat in cat_names:
            ...
            if st.session_state.editing_cat == cat:
                ...
                with e2:
                    if st.button("💾", key=f"sv_{cat}", width="stretch"):
                        rn = rn.strip()
                        if rn and rn != cat:
                ...
                with c5:
                    if st.button("🗑️", key=f"dc_{cat}", width="stretch"):
